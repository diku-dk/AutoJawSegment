...
def predict_volume(model, X, batch_size=8, axis=0, sub_task=False, n_chunks=1):
    """
    TODO
    """
    print("Predicting...")
    ...
    if sub_task:
        pred, pred_sub = pred
        pred = np.moveaxis(pred, source=0, destination=axis)
        pred_sub = np.moveaxis(pred_sub, source=0, destination=axis)
        return [pred, pred_sub]
...
def map_real_space_pred(pred, grid, inv_basis, voxel_grid_real_space, method="nearest"):
    ...
    pred_shape_last = pred.shape[-1]
    pred_type = pred.dtype

    del pred, grid, fill

    t_shape = voxel_grid_real_space[0].shape

    # voxel_grid_real_space = mgrid_to_points(voxel_grid_real_space).T

    voxel_grid_real_space = np.reshape(voxel_grid_real_space,
                                       (voxel_grid_real_space.shape[0], -1))
    inv_basis = inv_basis.astype(np.float32)
    voxel_grid_real_space = inv_basis.dot(voxel_grid_real_space).T

    # voxel_grid_real_space = points_to_mgrid(voxel_grid_real_space, t_shape)
    # voxel_grid_real_space = voxel_grid_real_space.astype(np.float32)

    voxel_grid_real_space = np.reshape(voxel_grid_real_space.T,
                                       (voxel_grid_real_space.shape[1], *t_shape,))


    # Prepare mapped pred volume
    mapped = np.empty(voxel_grid_real_space[0].shape + (pred_shape_last,),
                      dtype=pred_type)
    ...
    inds = np.arange(voxel_grid_real_space.shape[1])
    result = pool.map(_do, voxel_grid_real_space[0], voxel_grid_real_space[1],
                      voxel_grid_real_space[2], inds)
...
def pred_3D_iso(model, sequence, image, extra_boxes, min_coverage=None):
    ...
    while not is_covered or not base_reached or not extra_reached:
        ...
        if np.max(pred) > 1:
            print(f'prediction max values should not exceed 1, got {np.max(pred)}')
    ...
    print(f'pred_vol max values should not exceed 1, got {np.max(pred_vol)}')
