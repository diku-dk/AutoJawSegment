...
class Augmenter(object):
    ...
    Not yet implemented
    """
    def __call__(self, batch_x, batch_y, bg_values, batch_w=None, batch_weight_map=None):
...
class Elastic(Augmenter):
    ...
    def __call__(self, batch_x, batch_y, bg_values, batch_w=None, batch_weight_map=None,
        ...
        augmented_z = [] if batch_weight_map is not None else None
        augmented_sub_y = [] if batch_sub_task is not None else None

        for i, (augment, x, y, bg_vals) in enumerate(zip(augment_mask,
                                                         batch_x, batch_y,
                                                  bg_values)):
            if batch_weight_map is not None:
                z = batch_weight_map[i]
            else:
                z = None

            if batch_sub_task is not None:
                sub_y = batch_sub_task[i]


            if augment:
                if batch_weight_map is not None:
                    x, y, z, sub_y = self.trans_func(x, y,
                                              self.alpha, self.sigma, bg_vals, z, sub_y)
            ...
            if batch_weight_map is not None:
                augmented_z.append(z)

            if batch_sub_task is not None:
                augmented_sub_y.append(sub_y)


        return augmented_x, augmented_y, batch_w, augmented_z, augmented_sub_y
