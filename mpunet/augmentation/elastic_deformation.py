...
def elastic_transform_2d(image, labels, alpha, sigma, bg_val=0.0,
                         weight_map=None, sub_label=None):
    """
    ...
    if weight_map is not None:
        ...
        if not mask_weight:
            weight_map_intrp = RegularGridInterpolator(coords, weight_map,
                                                method="linear",
                                                bounds_error=False,
                                                fill_value=0.0,
                                                dtype=np.float32)
        ...
        weight_map = weight_map_intrp(indices).reshape(shape).astype(weight_map.dtype)

    if sub_label is not None:
        # import pdb; pdb.set_trace()
        sub_label_intrp = RegularGridInterpolator(coords, sub_label,
                                            method="nearest",
                                            bounds_error=False,
                                            fill_value=0.0,
                                            dtype=np.uint8)

        sub_label = sub_label_intrp(indices).reshape(shape).astype(sub_label.dtype)


    # Interpolate and return in image shape
    return image, labels, weight_map, sub_label
