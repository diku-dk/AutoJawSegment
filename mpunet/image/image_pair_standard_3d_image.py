...
class ImagePair(object):
    ...
    """
    def __init__(self, img_path, labels_path=None, weight_path=None,
                 sample_weight=1.0,
                 bg_class=0, logger=None, im_dtype=np.float32,
                 lab_dtype=np.uint8,
                 volume_level_thresh=False, volume_level_normalization=False):
        """
    ...
    def __str__(self):
        return "ImagePair(id={}, shape={}, real_shape={}, loaded={})".format(
            self.identifier, self.shape, self.shape, self.is_loaded
        )
    ...
    def image(self):
        ...
        if self._image is None:
            self._image = self.load_image(self.image_path)
    ...
    def labels(self):
        ...
        if self._labels is None:
            try:
                self._labels = self.load_image(self.labels_path)
    ...
    def weight_maps(self):
        ...
        if self._weight_maps is None:
            try:
                self._labels = self.load_image(self.weight_path)
            except AttributeError:
                return None
    ...
    def shape(self):
        ...
        """
        s = self.load_shape(self.image_path)
        if len(s) == 3:
            s = np.append(s, 1)
        return s

    def load_shape(self, image_path):
        pass
