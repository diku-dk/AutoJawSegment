...
class ImagePairLoader(object):
    ...
    def __init__(self,
                 ...
                 no_log=False,
                 weight_map=False,
                 volume_level_thresh=False,
                 volume_level_normalization=False,
                 sub_task=False,
                 sub_task_subdir='surface',
                 identifier=None,
                 **kwargs):

        # minus in init in ImagePairLoader
        ...
        self.sub_task_subdir = sub_task_subdir
        ...
        if not initialize_empty:
            ...
            if not predict_mode:
                ...
                if self.weight_map:
                    try:
                        self.weight_paths = self.get_label_paths(img_subdir,
                                                        weight_subdir)
                    except:
                ...
                if self.sub_task:
                    try:
                        self.sub_task_paths = self.get_label_paths(img_subdir,
                                                        sub_task_subdir)
                    except:
                        self.sub_task_paths = None
            else:
        ...
        if self.predict_mode:
            for img_path in self.image_paths:
                image = ImagePair(img_path,
                                  ...
                                  bg_class=bg_class,
                                  logger=self.logger,
                                  volume_level_thresh=self.volume_level_thresh,
                                  volume_level_normalization=self.volume_level_normalization,
                                  )
                image_objects.append(image)
        else:
            if self.weight_map:
                for img_path, label_path, weight_path in zip(
                        self.image_paths, self.label_paths, self.weight_paths):
                ...
                for img_path, label_path, sub_task_path in zip(
                        self.image_paths, self.label_paths, self.sub_task_paths):
                    image = ImagePair(img_path, label_path,
    ...
    def add_images(self, image_pair_loader):
        ...
        """
        try:
            for img in image_pair_loader.images:
                self.images += img
                self._id_to_image = self.get_id_to_images_dict()
