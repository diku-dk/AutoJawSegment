...
class ImagePair(object):
    ...
    """
    def __init__(self, img_path, labels_path=None, weight_path=None, sub_task_path=None,
                 sample_weight=1.0,
                 bg_class=0, logger=None, im_dtype=np.float32,
                 lab_dtype=np.uint8, volume_level_thresh=False, volume_level_normalization=False,
                 **kwargs):
        """
        ...
        self.sub_task_path = None

        if not self.predict_mode:
            self.labels_path = self._validate_path(labels_path)
            if weight_path is not None:
                self.weight_path = self._validate_path(weight_path)

            if sub_task_path is not None:
                self.sub_task_path = self._validate_path(sub_task_path)
        ...
        if not self.predict_mode:
            ...
            if weight_path is not None:
                self.weight_obj = nib.load(self.weight_path)

            if sub_task_path is not None:
                self.sub_task_obj = nib.load(self.sub_task_path)
        ...
        self.volume_level_thresh = volume_level_thresh
        self.volume_level_normalization = volume_level_normalization
    ...
    def image(self):
        ...
        if self._image is None:
            self._image = self.image_obj.get_fdata(caching='unchanged',
                                                   dtype=self.im_dtype)
            if self.volume_level_thresh:
                 self._image = self._image * (self._image > 0)

            elif self.volume_level_normalization:
                self._image = (self._image - np.average(self._image))/(np.percentile(self._image, 75) -
                                                                       np.percentile(self._image, 25))
    ...
    def weight_maps(self):
        """ Like self.image """
        if self._weight_maps is None:
            try:
                self._weight_maps = self.weight_obj.get_fdata(
                    caching="unchanged").astype(self.im_dtype)
            except AttributeError:
                return None
        return self._weight_maps


    @property
    def sub_labels(self):
        """ Like self.image """
        if self._sub_labels is None:
            try:
                self._sub_labels = self.sub_task_obj.get_fdata(
                    caching="unchanged").astype(self.lab_dtype)
            except AttributeError:
                return None
    ...
    def get_interpolator_with_current(self):
        ...
        if not self.predict_mode:
            ...
            if self.sub_task_path is not None:
