...
class IsotrophicLiveViewSequence3D(IsotrophicLiveViewSequence):
    ...
    def __getitem__(self, idx):
        ...
        # print(f'before aug {len(batch_x)}')
        # Apply augmentation if specified

        augmented = self.augment(batch_x, batch_y,
                                                 batch_w, bg_values)

        batch_x, batch_y, batch_w = augmented[:3]

        # a = self.augment(batch_x, batch_y, batch_w, bg_values)
        
        # batch_x, batch_y, batch_w = self.augment(batch_x, batch_y,
        #                                          batch_w, bg_values)
        # print(f'after aug {len(batch_x)}')
