...
class IsotrophicLiveViewSequence(BaseSequence):
    ...
    def augment(self, batch_x, batch_y, batch_w, bg_values,
                batch_weight_map=None, batch_sub_task=None):
        # Apply further augmentation?
        if self.list_of_augmenters:
            for aug in self.list_of_augmenters:
                batch_x, batch_y, batch_w, batch_weight_map, batch_sub_task = aug(
                                                batch_x=batch_x,
                                                batch_y=batch_y,
                                                batch_w=batch_w,
                                                bg_values=bg_values,
        ...
        return batch_x, batch_y, batch_w, batch_weight_map, batch_sub_task
    ...
    def prepare_batches(self, batch_x, batch_y, batch_w, flatten_w=False,
        ...
        if batch_weight_map is not None:
            ...
            additional_term = 1 if not mask_weight else 0

            batch_w = batch_w * (additional_term + batch_weight_map)

            # batch_w = batch_w * batch_weight_map
        ...
        if batch_sub_task is not None:
            batch_sub_task = np.asarray(batch_sub_task)
        ...
        if self.flatten_y:
            ...
            if batch_sub_task is not None:
                batch_sub_task = batch_sub_task.reshape((len(batch_sub_task), -1, 1))

        elif batch_y.shape[-1] != 1:
            batch_y = np.asarray(batch_y).reshape(batch_y.shape + (1,))
            if batch_sub_task is not None:
                if batch_sub_task.shape[-1] != 1:
                    batch_sub_task = np.asarray(batch_sub_task).reshape(batch_sub_task.shape + (1,))

        # breakpoint()

        batch_y = np.array([batch_y, batch_sub_task]) if batch_sub_task is not None else batch_y
        # batch_w = np.array([batch_w, batch_w]) if batch_sub_task is not None else batch_w

        # batch_y = np.moveaxis(batch_y, 0, 1)

        
        # batch_y = [batch_y, batch_sub_task] if batch_sub_task is not None else batch_y
