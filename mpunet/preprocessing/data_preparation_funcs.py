...
def create_views_manual(views=6):
    base_path = './'

    from mpunet.interpolation.sample_grid import get_fixed_views
    views = get_fixed_views(views)

    np.savez(os.path.join(base_path, "views"), views)
...
def _base_loader_func(hparams, just_one, no_val, logger, mtype):
    ...
    weight_map = hparams['fit']['weight_map']
    sub_task = hparams['fit']['sub_task']
    # breakpoint()

    # Get data loaders
    train_data = ImagePairLoader(logger=logger, weight_map=weight_map,
                                 weight_subdir=hparams['fit']['distance_map_folder'],
                                 sub_task=sub_task,
                                 sub_task_subdir=hparams['fit']['surface_folder'],
                                 **hparams["train_data"])

    val_data = ImagePairLoader(logger=logger, weight_map=weight_map,
                               weight_subdir=hparams['fit']['distance_map_folder'],
                               sub_task=sub_task,
                               sub_task_subdir=hparams['fit']['surface_folder'],
                               **hparams["val_data"])
    ...
    if hparams.get("aug_data"):
        ...
        if "include" not in aug_data:
            ...
            augment_data = ImagePairLoader(logger=logger,
            ...
            train_data.add_images(augment_data)
...
def load_or_create_views(hparams, continue_training, logger, base_path, auditor):
    ...
    if not continue_training and not os.path.isfile(os.path.join(base_path, "views.npz")):
...
def prepare_for_multi_view_unet(hparams, just_one=False, no_val=False,
    ...
    print(f'weight_map in prepare_for_multi  {hparams["fit"]["weight_map"]}')
...
def prepare_for_multi_view_2_path_unet(hparams, just_one=False, no_val=False,
                                       continue_training=False, logger=None,
                                       base_path='./'):
    """
    TODO
