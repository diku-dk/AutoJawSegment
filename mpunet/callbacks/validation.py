...
class Validation(Callback):
    ...
    def __init__(self, val_sequence, steps, logger=None, verbose=True,
                 ignore_class_zero=True, n_classes=None, task_names=None,
        ...
        self.n_classes = self.data.n_classes if n_classes is None else n_classes
    ...
    @staticmethod
    def _count_cm_elements_from_queue_no_thread(
              steps,
              TPs,
              pred, true,
              relevant,
    ...
    def evalaute(self):
        ...
        if not self.use_multi_process:
            for i in range(self.steps):
                ...
                if len(pred) > 1:
                    y = [y[0][i] for i in range(len(pred))]

                for p_task, y_task, task, w_task in zip(pred, y, self.task_names, w):
                    # Run all metrics
                    for metric, name in zip(metrics, metrics_names):
                        if 'IoU' in str(metric):
                            continue

                        # breakpoint()
                        if name == 'sparse_categorical_accuracy':
                            m = tf.reduce_mean(metric(y_task, p_task, w_task))
                        else:
