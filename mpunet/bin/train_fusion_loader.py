...
def _run_fusion_data_to_disk(sets, logger, hparams, min_val_images, is_validation,
    ...
    for _round, _set in enumerate(sets):
        ...
        fusion_dataset_train = os.path.join(basedir, 'fusion_dataset_train')
        mkdir_safe(fusion_dataset_train)
        for i in range(N_im):
            image_i_fusion_dir = os.path.join(fusion_dataset_train, str(i))
        ...
        for num_im, image_id in enumerate(list(image_set_dict.keys())):
            ...
            image_i_fusion_dir = os.path.join(fusion_dataset_train, str(num_im))
            ...
            with seq.image_pair_queue.get_image_by_id(image_id) as image:
                ...
                #                   dtype=np.float32)
                points = np.empty(shape=(len(targets), n_classes),
                                  dtype=np.float32)

                points.fill(np.nan)

                fusion_label_name = 'fusion_label.csv'
                fusion_label_path = os.path.join(image_i_fusion_dir, fusion_label_name)
                np.savetxt(fusion_label_path, targets)
                ...
                for k, v in enumerate(views):
                    ...
                    points= predict_and_map(  model=unet,
                                              seq=seq,
                                              image=image,
                                              view=v,
...
def _run_fusion_training(sets, logger, hparams, min_val_images, is_validation,
        ...
        from funsion_generator import FusionGenerator
        import tensorflow as tf
        gens = FusionGenerator(batch_size=fm_batch_size,
                               base_dir="data_folder/train",
                               n_classes=n_classes,
                               )
        dim_patch = (fm_batch_size, 1)

        dataset = tf.data.Dataset.from_generator(
            generator=gens,
            output_types=(tf.float32, tf.int8),
            output_shapes=(dim_patch, dim_patch)
        )
...
def entry_func(args=None):
    ...
    delete_fusion_after = args['delete_fusion_after']
    only_save_to_disk = args['only_save_to_disk']
    ...
    try:
        _run_fusion_data_to_disk(sets, logger, hparams, min_val_images,
        ...
        if delete_fusion_after and os.path.exists(fusion_dataset_train):
            import shutil
            shutil.rmtree(fusion_dataset_train)
