...
def get_argparser():
    ...
    parser.add_argument("--extra", type=str, default='0',
    ...
    parser.add_argument("--probability_map", action='store_true',
                        help='Overwrite previous results at the output folder')
    return parser
...
def entry_func(args=None):
    ...
    probability_map = args["probability_map"]
    ...
    from mpunet.utils import get_best_model, create_folders, \
                                    pred_to_class, await_and_set_free_gpu, set_gpu, pred_to_probabilities
    from mpunet.utils.fusion import pred_3D_iso

    from mpunet.sequences.patch_sequence_3d import predict_3D_patches, predict_3D_patches_binary
    ...
    for n_image, image_id in enumerate(image_ids):
        ...
        with seq.image_pair_queue.get_image_by_id(image_id) as image_pair:
            ...
            if probability_map:
                p = pred_to_probabilities(pred, img_dims=3, has_batch_dim=False, softmax=True)
