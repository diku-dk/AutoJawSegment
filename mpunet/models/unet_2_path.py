...
class UNet_2_path(Model):
    ...
    def init_model(self):
        ...
        Up-sampling
        """
        bn_origin = self._create_upsample(bn, residual_cons, filters, kr)

        bn_surface = self._create_upsample(bn, residual_cons, filters, kr, name='upsample_surf')
        ...
        final_conv_origin_name = 'final_conv_origin'
        final_conv_surf_name = 'final_conv_surface'

        if self.n_classes == 1:
            out = Conv2D(self.n_classes, 1, activation='sigmoid', name=final_conv_origin_name)(bn_origin)

        else:
            out = Conv2D(self.n_classes, 1, activation=self.out_activation, name=final_conv_origin_name)(bn_origin)


        out_surface = Conv2D(1, 1, activation='sigmoid', name=final_conv_surf_name)(bn_surface)
