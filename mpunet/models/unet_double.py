...
from tensorflow.keras.layers import Input, BatchNormalization, Cropping2D, \
    ...
    UpSampling2D, Reshape, Add
import numpy as np
from mpunet.models.utils import *

class UNetDouble(Model):
    """
    ...
    def _create_encoder(self, in_, init_filters, kernel_reg=None,
        ...
        for i in range(self.depth):
            ...
            # conv = DoubleAttentionSeq(input_channel=int(filters * self.cf),
            #                           name=l_name + 'DoubleAttention1')(conv)
            ...
            conv = DoubleAttention(input_channel=int(filters * self.cf),
                                      name=l_name + 'DoubleAttention2')(conv)

            in_ = MaxPooling2D(pool_size=(2, 2), name=l_name + "_pool")(conv)
    ...
    def _create_bottom(self, conv, filters, kernel_reg=None, name="bottom"):
        conv = Conv2D(int(filters * self.cf), self.kernel_size,
                      activation=self.activation, padding=self.padding,
                      kernel_regularizer=kernel_reg,
                      name=name + "_conv1")(conv)

        # conv = DoubleAttentionSeq(input_channel=int(filters * self.cf),
        #                         name=name + 'DoubleAttention1')(conv)
        ...
        conv = DoubleAttention(input_channel=int(filters * self.cf),
                                name=name + 'DoubleAttention2')(conv)
    ...
    def _create_upsample(self, in_, res_conns, filters, kernel_reg=None,
        ...
        for i in range(self.depth):
            ...
            # if self.build_res:
            #     res = Conv2D(int(filters * self.cf), 1,
            #                  activation=self.activation, padding=self.padding,
            #                  kernel_regularizer=kernel_reg,
            #                  name=l_name + "_conv1_res")(up)
            #     conv = Add()([conv, res])

            conv = BatchNormalization(name=l_name + "_BN1")(conv)

            # Crop and concatenate
            cropped_res = self.crop_nodes_to_match(residual_connections[i], conv)

            cropped_res = DoubleAttention(input_channel=int(filters * self.cf),
                                      name=l_name + 'DoubleAttention1')(cropped_res)

            conv = DoubleAttention(input_channel=int(filters * self.cf),
                                      name=l_name + 'DoubleAttention2')(conv)

            conv = Concatenate(axis=-1,
                                name=l_name + "_concat")([cropped_res, conv])
            ...
            in_ = DoubleAttention(input_channel=int(filters * self.cf),
                                      name=l_name + 'DoubleAttention3')(in_)
