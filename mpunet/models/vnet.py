...
class VNet(Model):
    ...
    def resBlock(self, conv, stage, keep_prob, stage_num=5, name="encoder"):  # 收缩路径
        ...
        for i in range(2 if stage >= 2 else 1):
        ...
        # conv = DoubleAttentionSeq(input_channel=self.init_filters * (2 ** (stage)), name=l_name +
        #                                                                                  'DoubleAttention')(conv)

            # print('conv_down_stage_%d:' %stage,conv.get_shape().as_list())#输出收缩路径中每个stage内的卷积
        # print('conv_add:',conv_add.get_shape().as_list())
        # conv_add = Dropout(keep_prob)(conv_add)
    ...
    def _create_bottom(self, in_, filters, kernel_reg=None, name="bottom"):
        ...
        conv = BatchNormalization(name=name + "_BN_1")(conv)
        conv = ReLU()(conv)

        # conv = SpatialAttention(kernel_size=5)(conv)

        # conv = DoubleAttention(input_channel=filters, name='DoubleAttention_1')(conv)
        ...
        conv = BatchNormalization(name=name + "_BN_2")(conv)
        conv = ReLU()(conv)

        # conv = DoubleAttention(input_channel=filters, name='DoubleAttention_2')(conv)


        #
        # conv = DoubleAttentionSeq(input_channel=filters, name='DoubleAttentionBottom')(conv)
